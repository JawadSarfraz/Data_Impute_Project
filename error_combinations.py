    ...
    if os.path.exists(original_file_path):
        ...
        for feature in features:
            ...
            for percentage in percentages:
                min_errors = {algo: {'RMSE': float('inf'), 'NRMSE': float('inf'), 'FeatureSet': ''} for algo in algorithms}
                ...
                for feature_set in feature_sets:
                    for algorithm in algorithms:
                        ...
                        # Average RMSE/NRMSE and update min_errors if this is the new minimum
                ...
                for algo, error_info in min_errors.items():
                    final_results.append([combination, feature, percentage, algo, error_info['RMSE'], error_info['NRMSE'], error_info['FeatureSet']])

# Create DataFrame and save to CSV
df_final = pd.DataFrame(final_results, columns=['Combination', 'Feature', 'Percentage', 'Algorithm', 'Min RMSE', 'Min NRMSE', 'FeatureSet Removal Scenario'])
output_file_path = os.path.join(output_dir, 'min_error_analysis_with_feature_combination.csv')
